...
def stream_authoritative_chat(
    ...
    *,
    ...
    try:
        context, visual_input = build_context(user_message)
        developer_message = _developer_message(context)

        if visual_input:
            print("\n[Vision]")
            print("Target:", visual_input["requested_target"])
            print("Capture source:", visual_input["source"])
            print("Fresh screenshot attached:")
            print(visual_input["screenshot_path"])
            ...
            user_content = [
                ...
                {
                    "type": "input_image",
                    "image_url": visual_input["image_url"],
                },
            ]
        else:
        ...
        with client.responses.stream(
            ...
            for event in stream:
                ...
                for sentence in accumulator.add(delta):
                    if on_sentence is not None:
                        on_sentence(sentence)

        trailing = accumulator.flush()

        if trailing and on_sentence is not None:
        ...
        if visual_input and visual_input.get("temporary", True):
            screenshot_path = visual_input.get("screenshot_path")

            if screenshot_path:
                try:
                    delete_visual_artifact(screenshot_path)
                    print("\n[Vision Cleanup]")
